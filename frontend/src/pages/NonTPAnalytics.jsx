...
export default function NonTPAnalytics() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("Error fetching Non-TP analytics:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const { inventoryList, nonTpStockValue, nonTpTotalBottles } = useMemo(() => {
    const list = []
    let val = 0
    let bottles = 0
    ...
    return { inventoryList: list, nonTpStockValue: val, nonTpTotalBottles: bottles }
  }, [rawInventories])
    ...
    filteredSales.forEach((sale) => {
      ...
      sale.items?.forEach((item) => {
        if (item.stockType === "TP") {
          ...
          if (!prodMap[pName]) {
            prodMap[pName] = { name: pName, quantity: 0, revenue: 0, stockType: "Non-TP Stock" }
          }
  ...
  }, [filteredSales, filteredPurchases, nonTpStockValue, nonTpTotalBottles])
  ...
  const handleExportExcel = async () => {
    const metrics = [
      { label: "Non-TP Total Sales", stockType: "Non-TP Stock", value: `₹ ${dashboard.nonTpSales.toLocaleString()}`, note: "Gross revenue generated" },
      { label: "Non-TP Bottles Sold", stockType: "Non-TP Stock", value: `${dashboard.nonTpUnitsSold} Bottles`, note: "Dispatched volume" },
      { label: "Live Non-TP Stock Value", stockType: "Non-TP Stock", value: `₹ ${dashboard.nonTpStockValue.toLocaleString()}`, note: `${dashboard.nonTpTotalBottles} bottles in storage` },
      { label: "Non-TP Revenue Share", stockType: "Non-TP Stock", value: dashboard.totalSales > 0 ? `${((dashboard.nonTpSales / dashboard.totalSales) * 100).toFixed(1)}%` : "0%", note: "Of total bar sales" },
    ...
    await exportStyledAnalyticsExcel({
      reportTitle: `NON-TP LIQUOR ANALYTICS (${timeRangeLabel.toUpperCase()})`,
      ...
      inventoryList,
      salesLog,
      filename: `SuryaBar_NonTP_Analytics_${timeRange}`,
    })
  }

  if (loading) return <div className="p-8 text-center text-slate-500 text-xs font-bold">Loading Non-TP analytics...</div>
  ...
  return (
    <div className="space-y-4 md:space-y-6 max-w-7xl mx-auto font-sans">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-4 md:p-6 rounded-2xl shadow-xs border border-slate-200/90">
        <div>
          <h2 className="text-lg sm:text-2xl font-black text-slate-900">
            Non-TP Commercial Analytics
          </h2>
          <p className="text-xs font-semibold text-slate-500 mt-0.5">
            Commercial channel performance • {timeRangeLabel} turnover, bottle volume & inventory valuation
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
        ...
        <div className="bg-white rounded-2xl p-3.5 sm:p-5 shadow-xs border border-slate-200/90 hover:border-slate-400 hover:shadow-sm transition-all">
          <div className="flex justify-between items-start">
            <span className="text-[10px] sm:text-[11px] font-black text-slate-400 uppercase tracking-wider">Non-TP Revenue</span>
            <span className="px-2 py-0.5 bg-purple-100 text-purple-800 rounded-md text-[10px] font-black">Commercial</span>
          </div>
          <p className="text-xl sm:text-2xl md:text-3xl font-black text-slate-950 mt-1 sm:mt-1.5 tracking-tight">
            ₹{dashboard.nonTpSales.toLocaleString()}
          </p>
        ...
        <div className="bg-white rounded-2xl p-3.5 sm:p-5 shadow-xs border border-slate-200/90 hover:border-slate-400 hover:shadow-sm transition-all">
          ...
          <p className="text-xl sm:text-2xl md:text-3xl font-black text-slate-950 mt-1 sm:mt-1.5 tracking-tight">
            {dashboard.nonTpUnitsSold.toLocaleString()} <span className="text-xs font-semibold text-slate-400">bottles</span>
          </p>
          <p className="text-xs font-bold text-slate-600 mt-1 sm:mt-2">
        ...
        <div className="bg-white rounded-2xl p-3.5 sm:p-5 shadow-xs border border-slate-200/90 hover:border-slate-400 hover:shadow-sm transition-all">
          ...
          <p className="text-xl sm:text-2xl md:text-3xl font-black text-slate-950 mt-1 sm:mt-1.5 tracking-tight">
            ₹{dashboard.nonTpStockValue.toLocaleString()}
          </p>
          <p className="text-xs font-bold text-emerald-700 mt-1 sm:mt-2">
            {dashboard.nonTpTotalBottles.toLocaleString()} bottles in storage
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3.5 sm:gap-6">
        {/* Top Selling Non-TP Products Bar Chart */}
        <div className="bg-white rounded-2xl p-4 sm:p-5 shadow-xs border border-slate-200/90 flex flex-col justify-between">
          <div>
            <h3 className="font-black text-slate-900 text-xs sm:text-sm mb-0.5">Top Selling Non-TP Bottles</h3>
            <p className="text-[11px] text-slate-500 font-medium mb-3">Highest revenue generating Non-TP liquor items</p>
          </div>
          
          {topProducts.length === 0 ? (
            <div className="h-56 sm:h-64 flex items-center justify-center text-slate-400 text-xs font-medium">
              No Non-TP sales recorded for this period
            </div>
          ) : (
        ...
        {/* Non-TP vs TP Revenue Share Donut Chart */}
        <div className="bg-white rounded-2xl p-4 sm:p-5 shadow-xs border border-slate-200/90 flex flex-col justify-between">
          <div>
            <h3 className="font-black text-slate-900 text-xs sm:text-sm mb-0.5">Channel Revenue Distribution</h3>
            <p className="text-[11px] text-slate-500 font-medium mb-3">Revenue split between Non-TP commercial and TP regulated excise stock</p>
          </div>
