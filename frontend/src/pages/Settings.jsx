...
export default function Settings() {
  ...
  const handleResetDatabase = async () => {
    ...
    try {
      const token = localStorage.getItem("surya_bar_token")
      const res = await fetch(`${API_BASE_URL}/api/backup/reset`, {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        ...
        setSavedToast("Database reset successfully! All transactions & stock cleared.")
        fetchDbStats()
      } else {
    ...
    <div className="space-y-4 md:space-y-6 max-w-4xl mx-auto font-sans pb-16">
      ...
      {role === "ADMIN" && (
        <div className="bg-white rounded-2xl p-5 sm:p-6 shadow-xs border border-blue-200/80 space-y-5 relative overflow-hidden mt-6">
          ...
          <div className="p-4 bg-rose-50/50 dark:bg-rose-950/20 rounded-2xl border border-rose-200 dark:border-rose-900 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mt-4">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-base">🗑️</span>
                <h4 className="text-xs font-black text-rose-900 dark:text-rose-300 uppercase tracking-wide">
                  Danger Zone: Clear All Transaction & Sample Data
                </h4>
              </div>
              <p className="text-xs text-rose-700/80 dark:text-rose-400 font-medium mt-0.5">
                Wipes all dummy sales bills, live stock counts, purchase shipments, and customer credit debts to start fresh. <strong>Your Admin & Cashier logins are 100% preserved.</strong>
              </p>
            </div>
            ...
            <button
              ...
              className="px-4 py-2.5 bg-rose-600 hover:bg-rose-700 active:scale-95 text-white rounded-xl text-xs font-black transition-all cursor-pointer shrink-0 shadow-xs"
            >
              Reset Database
            </button>
          </div>
      ...
      {showResetModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl border border-slate-200 dark:border-slate-800 text-center space-y-4 animate-in zoom-in-95 duration-150">
            <div className="w-14 h-14 bg-rose-100 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 rounded-2xl flex items-center justify-center mx-auto text-2xl font-black border border-rose-200 dark:border-rose-800">
              ⚠️
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                This will permanently delete all test sales receipts, purchase entries, live cellar bottles, and customer khata ledgers.
              </p>
              <div className="mt-3 p-3 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-xl text-left text-xs font-medium text-emerald-800 dark:text-emerald-300">
                ✓ <strong>Preserved:</strong> Admin (<span className="font-mono">admin@suryabar.com</span>) and Cashier logins will remain active.
              </div>
            </div>
            ...
            <div className="flex gap-2.5 pt-2">
              ...
              <button
                ...
                disabled={resetting}
                className="flex-1 py-3 bg-rose-600 hover:bg-rose-700 active:scale-95 text-white font-black rounded-xl text-xs shadow-xs transition-all disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2"
              >
                {resetting ? "Wiping Data..." : "Confirm & Wipe Database"}
              </button>
            </div>
          </div>
        </div>
