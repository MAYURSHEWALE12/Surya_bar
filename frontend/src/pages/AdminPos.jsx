...
export default function AdminPos() {
  ...
  const addToCart = (product, stockType) => {
    const maxStock =
      stockType === "TP"
        ? Number(product.tp?.quantity) || 0
        : Number(product.nonTp?.quantity) || 0

    if (maxStock <= 0) {
      alert(`Out of stock for ${product.name} (${stockType})!`)
      return
    }

    const existingIndex = cart.findIndex(
      (item) => item.productId === product._id && item.stockType === stockType
    )

    const price =
      stockType === "TP"
        ? product.tp?.sellingPrice || 0
        : product.nonTp?.sellingPrice || 0

    if (existingIndex > -1) {
      if (cart[existingIndex].quantity >= maxStock) {
        alert(`Cannot add more. Only ${maxStock} units of ${product.name} (${stockType}) available in stock!`)
        return
      }
      const updated = [...cart]
        ...
        .map((item) => {
          if (item.productId === productId && item.stockType === stockType) {
            ...
            if (delta > 0 && newQty > item.maxStock) {
              alert(`Cannot exceed available stock (${item.maxStock} units)!`)
              return item
            }
  ...
  const removeFromCart = (productId, stockType) => {
    setCart(cart.filter((item) => !(item.productId === productId && item.stockType === stockType)))
  }
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-[calc(100vh-5.5rem)] w-full overflow-hidden relative">
              ...
              {filteredProducts.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Tactile Split POS Buttons with low stock indicators */}
      ...
      {/* Desktop Right Column: Cart & Checkout (hidden on mobile) */}
